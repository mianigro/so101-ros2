...
2. ``async_infer.launch.py`` — the configured VLA checkpoint drives the
...
DEFAULT_EXPERIMENT = "self_improve"
...
def session_commands(config: RoundConfig, *, setup: str,
    ...
    return [
        ...
        ["ros2", "launch", "so101_inference", "async_infer.launch.py",
         f"setup:={setup}",
         f"policy_type:={config.train.policy_type}",
         f"repo_id:={checkpoint}",
         f"server_address:={policy_server_address}",
         f"task:={config.rollout.task_prompt}",
         f"actions_per_chunk:={config.rollout.actions_per_chunk}"],
...
def run_post(config: RoundConfig, rounds_root: Path, *, input_dir: Path,
    ...
    judge_cmd = [
        sys.executable,
        str(REPOSITORY_ROOT / "self-improve" / "judge_rollouts.py"),
...
def main(argv: Optional[list] = None) -> int:
    ...
    post.add_argument("--repo-id", default=None,
                      help="converted dataset repo id "
                           "(default: local/so101_self_improve_round<N>_real)")
    ...
    repo_id = args.repo_id or (
        f"local/so101_self_improve_round{config.round_index}_real"
    )
