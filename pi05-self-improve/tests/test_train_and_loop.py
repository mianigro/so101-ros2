...
from pi05_selfimprove.config import RoundConfig
from pi05_selfimprove.train_bc import (
    build_train_command,
...
class TrainCommandTests(unittest.TestCase):
    ...
    def test_round_n_from_previous_checkpoint_on_mixed(self) -> None:
        ...
        self.assertIn("--dataset.repo_id=local/so101_pi05_round2_mixed",
                      joined)
    ...
    def test_round_n_from_base_on_mixed_with_rename_map(self) -> None:
        ...
        joined = " ".join(command)
        self.assertIn("--policy.path=lerobot/pi05_base", joined)
        self.assertIn("--dataset.repo_id=local/so101_pi05_round1_mixed",
                      joined)
        self.assertIn(
...
class LoopImportTests(unittest.TestCase):
    def test_stage_validation(self) -> None:
        from pi05_selfimprove.loop import ALL_STAGES, DEFAULT_STAGES
    ...
    def test_visualizer_is_forwarded_to_sim_rollout(self) -> None:
        from pi05_selfimprove.loop import _run_sim_rollout

        config = RoundConfig(round_index=1)
        with tempfile.TemporaryDirectory() as tmp, mock.patch(
                "pi05_selfimprove.loop.subprocess.run") as subprocess_run:
